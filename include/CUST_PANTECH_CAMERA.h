...
#define F_PANTECH_CAMERA
#define F_PANTECH_CAMERA_TARGET_EF65L
#define F_PANTECH_CAMERA_EF39S_40K
...
#ifdef F_PANTECH_CAMERA_LIVESNAPSHOT_PICTURESIZE
#define F_PANTECH_CAMERA_LIVESNAPSHOT_ROTATION /*201201005.WSH, Camera livesnapshot 가로/세로모드 촬영사진 rotaton적용.  */
#endif
...
#ifdef F_PANTECH_CAMERA_TARGET_EF65L                                                                                                                                                                                                                                      
/* LGT의 경우 MMS전송용 동영상촬영시 bitrate를 128bps로 사용해야 함.
  ...
  * xml파일이 있는 경우 아래 경로의 xml파일에서 수정해야 한다
  * LINUX\android\vendor\qcom\android-open\mediaprofiles\media_profiles.xml
*/
#define F_PANTECH_CAMERA_CFG_MMS_REC_FOR_LGT
#endif

/* 20120702.WSH,  EF34K Camera 정량화 Test에서 ANR발생 예방 code.
    initpreview()에서 preview thread, frame thread 생성시 frame thread가 너무 빨리 생성되어, camframe.c에서 시그널 sub_thread_ready_cond
    를 받지 못해 ANR발생. */ 
#define F_PANTECH_CAMERA_FRAME_THREAD_WAIT

/* 20120516. WSH, EF34K ICS Headset통화중 사진 촬영할경우 사진촬영음 Speaker로 송출되게 수정함.*/
#define F_PANTECH_CAMERA_PLAYSOUND_IN_CALL

/* 20120426. WSH, VT Call --> Chatting 화면 --> SKY입력기 설정 화면에서 상대폰에 video 영상 송출하지 못하는 현상 수정. */
#define F_PANTECH_CAMERA_VT_ICS_CHATTING
...
/* SKYCAM_PSJ_121205
 * 캠코더 녹화시 AV sync안맞는 문제 수정. 
 * EF33S, EF34K에서 적용된 내용 테스트시 정상 동작
*/
#define F_PANTECH_CAMERA_QBUG_ENCODING_AV_SYNC

/* SKYCAM_PSJ_121205
 * 캠코더 녹화시 녹화 종료음 포함 되는 문제 수정.
 * 사운드 파일의 앞부분을 묵음 처리하여 적용.
*/
#define F_PANTECH_CAMERA_FIX_REC_BEEP_SOUND

/* SKYCAM_PSJ_121213
 * 미니카메라 분할 촬영시 뮤텍스에 의한 Thread Dead Lock발생
 * DataCallback 세팅하여 사용 할 경우 비주기적으로 나올 수  있음
 * TakePicture의 mLock과 DataCallback내의 disableMessage의 mLock이 충돌하여 PreviewThread가 종료하지 못하고
 * 이로 인해 TakePicture내의 StopPreviewInternal함수가 Wating되고 있음
 * 이미 StopPreviewInternal이 동작한 상황이라면 DataCallback을 호출 하지 않도록 수정
*/
#define F_PANTECH_CAMERA_FIX_CONFRICT_MUTEX_WITH_DATACALLBACK



/* 20120604.WSH,  camera진입 속도를 높이기 위해 userbuild시 camera kernel/hal단 log중 error발생 제외하고 나머지 출력 안되도록 수정함.  */
#ifdef T_BUILD_USER
#define F_PANTECH_CAMERA_KERNEL_LOG_BUILD_MODE
#endif
...
#endif
#ifdef F_PANTECH_CAMERA_TARGET_EF65L                                                                                                                                                                                                                                      
#define C_PANTECH_CAMERA_EXIF_MODEL     "IM-A820L"
#define C_PANTECH_CAMERA_EXIF_MODEL_LEN     9       /* including NULL */
#endif
#endif
