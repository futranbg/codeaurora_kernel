...
#ifdef CONFIG_PANTECH_ERR_CRASH_LOGGING
		pr_err("NORMAL_BOOT_DONE status : %d\n", status);
		if(status){
			pr_err("%s: 134 gpio_get_value: %d\n", __func__, gpio_get_value(134));
...
static void check_reset_and_mdm_ramdump_on(void)
{
	...
	mdmdump = readl(restart_reason + 0x10);
	iounmap(restart_reason);

    if(mdmdump == PANTECH_MDMDUMP_MAGIC)
    {
		switch (reason)
		{
...
static int __init charm_modem_probe(struct platform_device *pdev)
{
	...
	if(is_reset_and_mdm_ramdump_on == 1){
		msleep(50); //p10919 change msleep 4sec -> 50ms for only ramdump dump
	}else{
		msleep(4000);
	}
