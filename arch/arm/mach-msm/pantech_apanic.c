...
// paiksun...
#ifdef CONFIG_SW_RESET
//pdj666 20120302 extern int pm8058_get_pwrkey_status(void);
extern int pm8xxx_get_pwrkey_status(void);
#endif
...
static int __init __init_pantech_debug_log(void)
{
	...
	vaddr = allocate_contiguous_ebi(debug_buf_size, PAGE_SIZE, 0);
	...
	printk(KERN_INFO "[%s]allocated mem for pantech debug : vaddr=0x%x size=0x%x \n", __func__, (unsigned int)vaddr, (unsigned int)debug_buf_size);

	memset(vaddr, 0, debug_buf_size);

	for (index = 0; index < CONFIG_NR_CPUS; index++) {
#ifdef CONFIG_PANTECH_DEBUG_SCHED_LOG  //p14291_121102
		atomic_set(&(vaddr->idx_sched[index]), -1);
#endif
#ifdef CONFIG_PANTECH_DEBUG_IRQ_LOG  //p14291_121102
		atomic_set(&(vaddr->idx_irq[index]), -1);
#endif
	}
	...
	pantech_dbg_log = (struct pantech_debug_log*)vaddr;
...
#ifdef CONFIG_PANTECH_DEBUG_SCHED_LOG  //p14291_121102
void __pantech_debug_task_sched_log(int cpu, struct task_struct *task, char *msg)
{
	unsigned i;
...
#endif  /*CONFIG_PANTECH_DEBUG_SCHED_LOG */
...
#endif  /* CONFIG_PANTECH_DEBUG_IRQ_LOG */
...
#endif /* CONFIG_PANTECH_DEBUG_DCVS_LOG  */
...
#endif  /* CONFIG_PANTECH_DEBUG_RPM_LOG  */
